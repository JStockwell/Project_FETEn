...
static func run(Combat, sefName:String, attacker, defender, accMod: int, critMod: int, spa: int = 0, imd = 0):
	match sefName:
		...
		"boost_1":
			boost_1(Combat, attacker, defender, accMod, critMod, spa, imd)
		"boost_2":
...
static func boost_1(Combat, attacker, defender, accMod: int, critMod: int, spa: int, imd: int):
	var rolls = Combat.generate_rolls()
	var rolls_retaliate = Combat.generate_rolls()
	
	if Combat.calc_hit_chance(attacker.get_dexterity(), defender.get_agility(), accMod + 15, rolls):
		var crit = Combat.calc_crit(attacker.get_dexterity(), attacker.get_agility(), defender.get_agility(), critMod + 3, rolls[3])
