...
func _ready():
	Player.set_stats(GameStatus_v4.activePlayer)
	Enemy.set_stats(GameStatus_v4.activeEnemy)
	
	setStatsLabel(playerStatsLabel, GameStatus_v4.activePlayer)
	setStatsLabel(enemyStatsLabel, GameStatus_v4.activeEnemy)
...
func attack_calculation(defender, attack, label, isPlayer):
	...
	if finalHP > 0:
		if isPlayer:
			GameStatus_v4.set_active_enemy_hp(finalHP)
		else:
			GameStatus_v4.set_active_player_hp(finalHP)
		change_phase()
		...
		if isPlayer:
			...
			GameStatus_v4.set_active_enemy_hp(0)
		else:
			phaseText.text = "YOU LOST"
			GameStatus_v4.set_active_player_hp(0)
		...
		if isPlayer:
			...
			for enemy in GameStatus_v4.enemies:
				enemyHealthPool += GameStatus_v4.enemies[enemy]["current_health"]
			
			if enemyHealthPool <= 0:
				get_tree().change_scene_to_file("res://proto_combat_v4/Scenes/victory.tscn")
			else:
				get_tree().change_scene_to_file("res://proto_combat_v4/Scenes/choose_combat.tscn")

		else:
			var playerHealthPool = 0
			for player in GameStatus_v4.party:
				playerHealthPool += GameStatus_v4.party[player]["current_health"]
				
			if playerHealthPool <= 0:
				print("HELP")
				get_tree().change_scene_to_file("res://proto_combat_v4/Scenes/lose.tscn")
			else:
				get_tree().change_scene_to_file("res://proto_combat_v4/Scenes/choose_combat.tscn")
...
func save_result():
	GameStatus_v4.party[GameStatus_v4.activePlayer["name"]] = GameStatus_v4.activePlayer
	var enemyId = GameStatus_v4.activeEnemy["id"]
	GameStatus_v4.activeEnemy.erase("id")
	GameStatus_v4.enemies[enemyId] = GameStatus_v4.activeEnemy
